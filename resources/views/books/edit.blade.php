...
@section('content')
    <form action="/books/{{$book->id}}" method="post" class="col-md-6 needs-validation" enctype="multipart/form-data" novalidate>
        @method('PUT')
        @csrf
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <input type="text" pattern="((^(\d+)(\.?)(\d+)$)|^(\d)$)" class="form-control" name="price" id="price"
                placeholder="Book price" required value="{{ $book->price }}" />
            @error('price')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="date" class="form-control" name="released_at" id="released_at" placeholder="Book released at"
                required value="@php echo $date->format('Y-d-m')@endphp" />
            @error('released_at')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        </div>
            ...
            <select class="form-select select2" placeholder="Book genres" name="genres[]" id="genres" size="1"
                ...
                @foreach ($genres as $genre)
                    <option value="{{ $genre->id }}"
                        @php echo $book->genres->contains($genre) ? 'selected' : '' @endphp>{{ $genre->name }}
                    </option>
                @endforeach
            </select>
        </div>
