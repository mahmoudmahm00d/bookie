<div class="p-5 mx-2 my-2 bg-white rounded border-2">
    @if ($content->count() > 0)
        @foreach ($content as $id => $item)
            <p class="text-right mb-2 d-flex">
                <span>{{ $item->get('title') }} x {{ $item->get('quantity') }}</span>
                <button class="ms-auto text-sm p-2 border rounded border bg-danger text-light"
        ...
        <a href="{{ url('/borrows/create') }}" class="btn btn-primary">Purchase</a>
        <button class="btn btn-outline-danger" wire:click="clearCart">Clear Cart</button>
    @else
        <div class="row">
            <svg xmlns="http://www.w3.org/2000/svg" class="mx-auto" data-name="Layer 1" width="150"
