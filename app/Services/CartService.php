...
class CartService
{
    ...
    {
        $cartItem = $this->createCartItem($id, $title, $price, $quantity, $options);

        $content = $this->getContent();

        if ($content->has($id)) {
            $cartItem->put('quantity', $content->get($id)->get('quantity'));
     ...
     * @return Illuminate\Support\Collection
     */
    protected function createCartItem(int $id, string $title, string $price, string $quantity, array $options): Collection
    {
